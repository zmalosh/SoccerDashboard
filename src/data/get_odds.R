...
get_odds_by_fixture <- function(fixtureId, allowCache = TRUE){
	url <- paste0('https://api-football-v1.p.rapidapi.com/v2/odds/fixture/', fixtureId)
	localPath <- paste0(getwd(), '/data/raw/odds/fixtureOdds_', str_pad(fixtureId, 4, pad = '0'), '.csv')
	cacheExpirationMin <- 24 * 60 # ODDS UPDATE ONCE PER DAY
	if(allowCache){
